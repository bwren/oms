...
foreach ($file in $files) {
  ...
  foreach ($link in $foundLinks) {
    if ($link.groups['link'].value.StartsWith('!')) {
        ...
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f $filelocation,$filename,'Media',$test,$file 
    }
    elseif ($link.groups['file'].value.EndsWith('.md')) {
        $file = $link.groups['file'].value
        $test = Test-Path "$reporoot\$file"
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f $filelocation, $filename,'Article',$test,$file 
    }
    elseif ($link.groups['file'].value.StartsWith('http')) {
        $url = $link.groups['uri'].value
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f  $filelocation, $filename,'Web URL',' ',$url
<#
        $r = resolveURL $url
        if ($url.tolower().Contains('go.microsoft.com/fwlink') -or $url.tolower().Contains('aka.ms')) {
            $outmessage = '{0} | {1,-8} | {2,-5} | {3}' -f $filename,'Redirect',' ',$url
        } else {
            $outmessage = '{0} | {1,-8} | {2,-5} | {3}' -f $filename,'Web URL',' ',$url
        }
        if ($r.status -ne 200) {
...
#>    
    ...
    elseif ($link.groups['file'].value.StartsWith('/')) {
        ...
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f $filelocation, $filename,'Docs URL',' ',$url
    }
    elseif ($link.groups['file'].value  -eq '' -and $link.groups['anchor'].value  -ne '') {
        $url = $link.groups['anchor'].value
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f $filelocation, $filename,'Anchor',' ',$url
    }
    else {
        '{0} | {1} | {2,-8} | {3,-5} | {4}' -f $filelocation, $filename,'Unknown',' ',$link.groups['link'].value
    }
  }
}
